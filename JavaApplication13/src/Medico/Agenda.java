...
/**
 * Classe Agente referente a agenda de um medico que tem a capassidade de pegar listas de Espedientes/Cirurgias, Marcar Conssultas/Retornos/Cirurgia, Excluir, tempos medios de primeiraConssulta e Retorno.
 * @author  Grupo 7
 *
 */
...
public class Agenda {
	...
	/**
	 * Constroi uma agenda, passando tempo da primeira consulta e da consulta de retorno
	 * @param tempoPrimeiraConsulta Tempo da primeira consulta
	 * @param tempoConsultaRetorno Tempo da consulta de retorno
	 */
	 ...
	 * @param tempoPrimeiraConsulta tempo de uma primeira consulta em minutos.
	 */
	...
	public HorarioDeAtendimento proximoDiaLivreConsulta(HorarioDeAtendimento horario){
		int dia = horario.getDiaDoMes();
		int diaDaSemana = horario.getDiaDaSemana();
		int mes = horario.getMes();
		int ano = horario.getAno();
		if(estaAtendendoHoje(diaDaSemana) && !horarioDoDiaEstaCompleto(dia, mes, ano)){
	 ...
	 * @param horario informa numero de identificacao do paciente
	...
	public HorarioDeAtendimento proximoDiaLivreCirurgia(HorarioDeAtendimento horario){
		...
		int mes = horario.getMes();
		int ano = horario.getAno();
		if(estaEmCirurgia(diaDaSemana) && cirurgiaMarcadaParaHoje(dia, mes, ano)){
			horario.setDiaDoMes(dia);
			horario.setAno(ano);
			horario.setMes(mes);
			return horario;
	 ...
	 * @param horario informa numero de identificacao do paciente e se a consulta eh de retorno ou primeira consulta
	 * @param calendarioNovo Marcar uma consulta em um horario vago apartir dessa data
	 * @return True caso tenha marcado com sucesso.
	 */
	public boolean marcarConsulta(HorarioDeAtendimento horario, Calendar calendarioNovo){
	 ...
	 * @param horario informa numero de identificacao do paciente e se a consulta eh de retorno ou primeira consulta
	 * @param calendarioNovo Data limite da marcacao da consulta
	 * @return True caso encontre alguma data ate a data limite, false caso constrario
	 */
	public boolean marcarConsultaComDataLimite(HorarioDeAtendimento horario, Calendar calendarioNovo){
		if (horario.getDiaDoMes() == calendarioNovo.get(Calendar.DAY_OF_MONTH) && horario.getMes() == calendarioNovo.get(Calendar.MONTH)){
			return false;
		}
	...
	public boolean cirurgiaMarcadaParaHoje(int diaDoMes, int mes, int ano){
		Iterator<HorarioDeAtendimento> iteraHorarios = this.getListaPacientesCirurgia().iterator();

		while (iteraHorarios.hasNext()) {
			HorarioDeAtendimento proximoCliente = iteraHorarios.next();
			if(proximoCliente.getMes() == mes && proximoCliente.getDiaDoMes()+1 == diaDoMes ){
				return false;
			}
		}
		return true;
	...
	public HorarioDeAtendimento excluirConsulta(HorarioDeAtendimento horario){
		...
		Iterator<HorarioDeAtendimento> it = this.getListaPacientesConsulta().iterator();
		
		while (it.hasNext()){
			horarioExcluido = it.next();
			if (horarioExcluido.getNumeroDeIdentificao() == horario.getNumeroDeIdentificao() && listaPacientes.remove(horario)){
	...
	/**
	 * Exclui uma cirurgia
	 * @param horario - Horario da cirurgia
	 * @return - Retorna a cirurgia removida
	 */
	public HorarioDeAtendimento excluirCirurgia(HorarioDeAtendimento horario){
		HorarioDeAtendimento horarioExcluido = null;
		Iterator<HorarioDeAtendimento> it = this.getListaPacientesCirurgia().iterator();
		
		while (it.hasNext()){
			horarioExcluido = it.next();
			if (horarioExcluido.getNumeroDeIdentificao() == horario.getNumeroDeIdentificao()){
	...
	public String toString(){
		...
		if (!listaPacientesCirurgia.isEmpty()){
			Iterator<HorarioDeAtendimento> it2 = listaPacientesCirurgia.iterator();
	...
	private boolean estaAtendendoHoje(int diaDaSemana){	
		Iterator<DiaDeTrabalho> iteraDiaDeTrabalho = this.getDiasDeExpediente().iterator();
	...
	private boolean horarioDoDiaEstaCompleto(int diaDoMes, int mes, int ano){
		Iterator<HorarioDeAtendimento> iteraHorarios = listaPacientes.iterator();
		...
		while (iteraHorarios.hasNext()) {
			HorarioDeAtendimento proximoCliente = iteraHorarios.next();
			if(proximoCliente.getMes() == mes && proximoCliente.getDiaDoMes() == diaDoMes ){
				tempoTotalDeConsultasPorDias += tempoDaConsulta(proximoCliente);
			}
		}
		
		Iterator<DiaDeTrabalho> iteraSemana = this.getDiasDeExpediente().iterator();
	...
	private boolean estaEmCirurgia(int diaDaSemana) {
		Iterator<DiaDeTrabalho> iteraDiasDeCirurgia = this.getDiasDeCirurgia().iterator();
